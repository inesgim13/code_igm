function [F_data,betas,A] = mkl_modified_igm(KERNELS,options, resume, resume_iterations)
...
if ~resume
    ...
    for i = 1:n_features
        K_sum = K_sum + KERNELS(i,:,:);
    end
    K_sum = K_sum / n_features;
    K_sum = reshape(K_sum, [N, N]);
